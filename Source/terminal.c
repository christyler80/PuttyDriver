...
/* PuttyDriver #8 - Putty has processed data. */
         if (curs_x != term->curs.x || curs_y != term->curs.y) {
            ...
            if (parent_hwnd > 0) {
                ...
                if (parent) {
                    ...
                    sprintf(buf, "#~#CUR2%04d %04d#~#", term->curs.x, term->curs.y);
...
/* PuttyDriver #9 - Used for Debugging / Message Tracing.
    if (parent_hwnd > 0) {
        HWND parent = GetWindow(parent_hwnd, GW_HWNDFIRST);
        if (parent) {
            char buf[20];
...
/* PuttyDriver #11 - Putty terminal is processing data. */
    /* putty_scrdata = true; */
    if (parent_hwnd > 0 && putty_scrdata == true) {
        HWND parent = GetWindow(parent_hwnd, GW_HWNDFIRST);
        if (parent) {
            COPYDATASTRUCT cd;
